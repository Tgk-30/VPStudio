...
struct ComponentConstructionVisionTests {
    ...
    @Test func detailHeroAndSeasonSectionsHostOnVisionOS() throws {
        ...
        let firstEpisode = Episode(
            id: "vision-series-s1e1",
            mediaId: "vision-detail-series",
            seasonNumber: 1,
            episodeNumber: 1,
            title: "Pilot",
        ...
        let hosted = try hostInVisibleVisionWindow(view.frame(width: 820, height: 780))
        defer { tearDownVisionWindow(hosted.window) }
        hosted.host.view.setNeedsLayout()
        hosted.host.view.layoutIfNeeded()
        RunLoop.main.run(until: Date().addingTimeInterval(0.05))
