...
@Suite(.serialized)
struct DownloadManagerTestsRootdownloadmanagertests {
    ...
    @Test func retryingTaskWithInvalidPersistedStreamURLFailsWithoutPerformerInvocation() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-invalid-stream-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        let callCounter = AttemptCounter()
        let performer: DownloadManager.DownloadPerformer = { _, _, _ in
        ...
        let persisted = DownloadTask(
            id: "invalid-url-task",
            mediaId: "tt-invalid-url",
            streamURL: ":::/not a valid url",
            fileName: "invalid-stream-url.mkv",
            totalBytes: 1_024
        )
        try await database.saveDownloadTask(persisted)
        ...
        #expect(failed.errorMessage?.contains("bad URL") == true)
        #expect(await callCounter.snapshot() == 0)
    }

    @Test func retryingHostlessPersistedStreamURLWithoutResumeDataSkipsPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-hostless-stream-url-no-resume.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let persisted = DownloadTask(
            id: "hostless-no-resume-task",
            mediaId: "tt-hostless-no-resume",
            streamURL: "http://",
            fileName: "hostless-no-resume.mkv",
            status: .failed,
            progress: 0.15,
            bytesWritten: 150,
            totalBytes: 1_000
    ...
    @Test func retryingMissingPersistedStreamURLWithoutRefresherFailsWithoutPerformerInvocation() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-missing-stream-url-no-refresher.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let recoveryContext = try #require(
            StreamRecoveryContext(
                infoHash: "missing-stream-no-refresher-context",
                preferredService: .realDebrid,
                seasonNumber: 1,
                episodeNumber: 1
            )
        ...
        let persisted = DownloadTask(
            id: "missing-no-refresher-task",
            mediaId: "tt-missing-refresher",
            streamURL: nil,
            fileName: "missing-stream-no-refresher.mkv",
            status: .failed,
            progress: 0.11,
            bytesWritten: 110,
            totalBytes: 1_000,
            mediaTitle: "Missing Stream",
            mediaType: "movie",
            recoveryContextJSON: try recoveryContext.jsonString()
        )
        try await database.saveDownloadTask(persisted)
    ...
    @Test func invalidPersistedStreamURLOnStartupDoesNotInvokePerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-invalid-stream-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let persisted = DownloadTask(
            id: "startup-invalid-url-task",
            mediaId: "tt-startup-invalid-url",
            streamURL: ":::///@@@",
            fileName: "startup-invalid-stream-url.mkv",
            status: .downloading,
            progress: 0.42,
            bytesWritten: 420,
            totalBytes: 1_024
    ...
    @Test func startupWithHostlessPersistedStreamURLWithoutResumeDataSkipsPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-hostless-stream-url-no-resume.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let persisted = DownloadTask(
            id: "startup-hostless-no-resume-url-task",
            mediaId: "tt-startup-hostless-no-resume",
            streamURL: "http://",
            fileName: "startup-hostless-no-resume.mkv",
            status: .downloading,
            progress: 0.3,
            bytesWritten: 300,
            totalBytes: 1_000
    ...
    @Test func missingPersistedStreamURLOnStartupUsesRefresherForResolution() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-missing-stream-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        let recoveryContext = try #require(
            StreamRecoveryContext(
                infoHash: "streamless-startup-context",
                preferredService: .realDebrid,
                seasonNumber: 4,
                episodeNumber: 8
            )
        )
        let refresherCallCounter = AttemptCounter()

        let persisted = DownloadTask(
            id: "startup-missing-stream-url",
            mediaId: "tt-startup-missing-url",
            streamURL: nil,
            fileName: "startup-missing-stream-url.mkv",
            status: .downloading,
            progress: 0.5,
            bytesWritten: 500,
            totalBytes: 1_000,
            mediaTitle: "Startup Missing Stream URL",
            mediaType: "series",
            seasonNumber: 4,
            episodeNumber: 8,
            recoveryContextJSON: try recoveryContext.jsonString()
        ...
        let manager = DownloadManager(
            ...
            linkRefresher: { _ in
                _ = await refresherCallCounter.next()
                return URL(string: "https://cdn.example.com/resolved-startup.mkv")!
            }
        )
        ...
        #expect(completed.recoveryContext?.infoHash == recoveryContext.infoHash)
        #expect(await refresherCallCounter.snapshot() == 1)
    }

    @Test func retryingTaskWithoutPersistedStreamURLUsesRefresher() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-missing-stream-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        let recoveryContext = try #require(
            StreamRecoveryContext(
                infoHash: "streamless-retry-context",
                preferredService: .realDebrid,
                seasonNumber: 2,
                episodeNumber: 11
            )
        ...
        let persisted = DownloadTask(
            id: "retry-missing-stream-url",
            mediaId: "tt-retry-missing-url",
            streamURL: nil,
            fileName: "retry-missing-stream-url.mkv",
            status: .failed,
            progress: 0.25,
            bytesWritten: 250,
            totalBytes: 1_000,
            mediaTitle: "Retry Missing Stream URL",
            mediaType: "series",
            seasonNumber: 2,
            episodeNumber: 11,
            recoveryContextJSON: try recoveryContext.jsonString()
        ...
        let manager = DownloadManager(
            ...
            linkRefresher: { _ in
                _ = await refresherCallCounter.next()
                return URL(string: "https://cdn.example.com/resolved-retry.mkv")!
            },
            resumePersistedDownloadsOnInit: false
        )
    ...
    @Test func retryingTaskWithHostlessPersistedStreamURLAndResumeDataUsesPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-resume-hostless-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        let attemptCounter = AttemptCounter()
        let resumeDataRecorder = ResumeDataRecorder()

        let persisted = DownloadTask(
            id: "hostless-resume-url-task",
            mediaId: "tt-resume-hostless",
            streamURL: "http://",
            fileName: "hostless-resume.mkv",
            status: .failed,
            progress: 0.15,
            bytesWritten: 150,
            totalBytes: 1_000,
            resumeDataBase64: Data("resume-data".utf8).base64EncodedString()
        )
        ...
        let manager = DownloadManager(
            ...
            performer: { _, _, _ in
                ...
                await resumeDataRecorder.record(Data("resume-data".utf8))
                let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
                let data = Data(repeating: 0x55, count: 1_024)
                try data.write(to: tempURL)
                let response = URLResponse(
                    url: URL(string: "https://cdn.example.com/resumed-hostless.mkv")!,
                    mimeType: "video/x-matroska",
        ...
        try await manager.retryDownload(id: persisted.id)
        let completed = try await waitForStatus(database: database, id: persisted.id, expected: .completed, timeoutSeconds: 10)
    ...
    @Test func startupWithHostlessPersistedStreamURLAndResumeDataUsesPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-resume-hostless-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let persisted = DownloadTask(
            id: "startup-hostless-resume-url-task",
            mediaId: "tt-startup-resume-hostless",
            streamURL: "http://",
            fileName: "startup-hostless-resume.mkv",
            status: .downloading,
            progress: 0.3,
            bytesWritten: 300,
            totalBytes: 1_000,
    ...
    @Test func retryingTaskWithFileStreamURLInvokesPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-file-url.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let persisted = DownloadTask(
            id: "file-url-task",
            mediaId: "tt-file-url",
            streamURL: "file:///tmp/example.mkv",
        ...
        let manager = DownloadManager(
            ...
            performer: { _, _, _ in
                ...
                try data.write(to: tempURL)
                let response = URLResponse(
                    url: URL(string: "file:///tmp/example.mkv")!,
                    mimeType: "video/x-matroska",
                    expectedContentLength: data.count,
                    textEncodingName: nil
                )
        ...
        try await manager.retryDownload(id: persisted.id)
        _ = try await waitForStatus(database: database, id: persisted.id, expected: .completed)
        #expect(await attemptCounter.snapshot() == 1)
    }
    ...
    @Test func invalidFileNamesAreSanitizedBeforeSaving() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-sanitized-filename.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let task = try await manager.enqueueDownload(
            stream: makeStream(name: "bad/file:name*?.mkv"),
            mediaId: "tt-invalid-name",
            episodeId: nil
        )

        let completed = try await waitForStatus(database: database, id: task.id, expected: .completed)
        let expectedFileName = "bad_file_name__.mkv"
        #expect(completed.fileName == expectedFileName)
        let storedPath = try #require(completed.destinationPath)
        #expect(URL(fileURLWithPath: storedPath).lastPathComponent == expectedFileName)
        #expect(!storedPath.contains("bad/file"))
        #expect(!storedPath.contains(":"))
        #expect(!storedPath.contains("*"))
        #expect(!storedPath.contains("?"))
    }

    @Test func filenameFromWhitespaceSanitizesToRandomDownloadName() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-whitespace-filename.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let task = try await manager.enqueueDownload(stream: makeStream(name: "   "), mediaId: "tt-whitespace", episodeId: nil)
        let completed = try await waitForStatus(database: database, id: task.id, expected: .completed)
        let sanitized = completed.fileName

        #expect(sanitized.hasPrefix("download-"))
        #expect(sanitized.hasSuffix(".mp4"))
        #expect(sanitized.contains(" ") == false)
        #expect(!sanitized.isEmpty)
        #expect(FileManager.default.fileExists(atPath: try #require(completed.destinationPath)))
    }

    @Test func dotFileNamesSanitizeToRandomDownloadName() async throws {
        let names = [".", ".."]
        let (database, rootDir) = try await makeDatabase(named: "download-manager-dot-filename.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        for name in names {
            let task = try await manager.enqueueDownload(
                stream: makeStream(name: name),
                mediaId: "tt-dot-\(name == "." ? "single" : "double")",
                episodeId: nil
            )
            let completed = try await waitForStatus(database: database, id: task.id, expected: .completed)
            let sanitized = completed.fileName
    ...
    @Test func distinctInvalidFileNamesCanStillCollideAfterSanitization() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-sanitized-collision.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        let expectedFirstName = "same_name_.mkv"
        let expectedSecondName = "same_name_ (1).mkv"

        let firstAttemptGate = BlockingDownloadGate()
        let attemptCounter = AttemptCounter()
        let performer: DownloadManager.DownloadPerformer = { _, _, _ in
            if await attemptCounter.next() == 1 {
                await firstAttemptGate.wait()
            }

            let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
            let data = Data(repeating: 0x22, count: 256)
            try data.write(to: tempURL)
            let response = URLResponse(
                url: URL(string: "https://cdn.example.com/video.mkv")!,
                mimeType: "video/x-matroska",
        ...
        let first = try await manager.enqueueDownload(
            stream: makeStream(name: "same/name?.mkv"),
            mediaId: "tt-sanitized-a",
            episodeId: nil
        )
        let second = try await manager.enqueueDownload(
            stream: makeStream(name: "same:name?.mkv"),
            mediaId: "tt-sanitized-b",
            episodeId: nil
        )
        await firstAttemptGate.resume()
        ...
        let firstName = URL(fileURLWithPath: try #require(firstCompleted.destinationPath)).lastPathComponent
        let secondName = URL(fileURLWithPath: try #require(secondCompleted.destinationPath)).lastPathComponent
    ...
    @Test func existingDestinationFileForcesCollisionSuffix() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-existing-file-collision.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        try FileManager.default.createDirectory(at: downloadsDir, withIntermediateDirectories: true)
        let preExisting = downloadsDir.appendingPathComponent("already-used.mkv")
        try Data([0x22]).write(to: preExisting)
        ...
        let task = try await manager.enqueueDownload(
            stream: makeStream(name: "already-used.mkv"),
            mediaId: "tt-existing-file",
            episodeId: nil
        )

        let completed = try await waitForStatus(database: database, id: task.id, expected: .completed)
        let completedPath = URL(fileURLWithPath: try #require(completed.destinationPath))

        #expect(completedPath.lastPathComponent == "already-used (1).mkv")
        #expect(completedPath.path != preExisting.path)
    ...
    @Test func retryDownloadWithoutTransportStateFailsWithoutInvokingPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-root-retry-no-transport-state.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func concurrentDownloadRejectsWhenEffectiveRemainingSpaceIsZero() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-disk-space-concurrent-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func retryDownloadFailsWhenDiskSpaceProviderThrowsDuringRetry() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-disk-space-provider-throw-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
        ...
        let manager = DownloadManager(
            ...
            availableDiskSpace: { _ in throw DownloadManagerTestError.timeout },
            resumePersistedDownloadsOnInit: false
        )
    ...
    @Test func startupRecoveryBackedQueuedTaskWithInvalidHostFailsWithoutRefresherOrPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-recovery-invalid-host-no-refresher-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func removeDownloadSucceedsWhenCompletedFileIsMissing() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-remove-missing-file-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func retryDirectDownloadUsesMutatedPersistedURLWhenNoCachedReplayStateExists() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-direct-mutate-persisted-url-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func startupQueuedTaskWithoutTransportStateFailsWithoutInvokingPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-no-transport-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func startupQueuedTaskWithInvalidHostFailsAsBadURLWithoutPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-invalid-host-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func cancelDownloadForMissingTaskDoesNotThrowOrCreateRows() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-cancel-missing-task-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func cancelDirectDownloadDoesNotInvokeRemoteCleanup() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-cancel-direct-no-cleanup-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }

        let downloadsDir = rootDir.appendingPathComponent("downloads", isDirectory: true)
        let cleanupCounter = AttemptCounter()
        ...
        #expect(await cleanupCounter.snapshot() == 0)
    }

    @Test func retryDownloadWithServerErrorDoesNotCallRefresher() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-server-error-no-refresh-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func genericTransferErrorDoesNotTriggerAdditionalLinkRefresh() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-generic-error-no-refresh-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func retryPersistedDownloadUsesPersistedResumeDataFromDatabase() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-retry-persisted-resume-data-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func directStreamRequestHeadersReachDownloadPerformer() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-direct-stream-headers-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func directStreamRequestHeadersSurviveRetry() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-direct-stream-retry-headers-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func directStreamRequestHeadersSurviveLinkRefreshRetry() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-direct-stream-refresh-headers-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func startupQueuedTaskFailsWhenDiskSpaceProviderThrows() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-disk-space-throw-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func startupRecoveryBackedQueuedTaskFailsWhenRefresherThrows() async throws {
        ...
        let (database, rootDir) = try await makeDatabase(named: "download-manager-startup-refresher-fail-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func fileURLStreamIsConsideredViableAndDownloads() async throws {
        let (database, rootDir) = try await makeDatabase(named: "download-manager-file-url-root.sqlite")
        defer { try? FileManager.default.removeItem(at: rootDir) }
    ...
    @Test func retryRecoveryBackedTaskWithRefresherDoesNotCacheTransportStateAcrossRetries() async throws {
        ...
        #expect(secondAttemptFailed.status == .failed)
        #expect(await attemptCounter.snapshot() == 2)
        #expect(await refreshCounter.snapshot() == 2)
